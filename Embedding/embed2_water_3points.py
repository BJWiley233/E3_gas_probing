...
def embed_event_pos(path, gas_resid, gas_idx, in_row, start, end, protein_cofactors,global_to_local,direction,coords_all):
    ...
    for idx in frame_indices:
        ...
        for cen, center in enumerate([gas_coords[0], mean_gas[gas_idx], gas_coords[1]]):
...
def embed_event_neg(path, gas_resid, gas_idx, in_row, start, end, protein_cofactors, global_to_local, direction,coords_all, n_candidates=512):
    ...
    for idx in frame_indices:
        ...
        candidate_schedule = [512, 1024, 1024, 5000, 15000]
        max_dist = 4.
        ...
        for w, (min_dist, n_candidates) in enumerate(zip(min_dists_list, candidate_schedule)):
            ...
            cand_O1 = gas_coords[0].unsqueeze(0) + translations
            cand_O2 = gas_coords[1].unsqueeze(0) + translations
            ...
            min_O1 = torch.cdist(cand_O1, atom_coords).min(dim=1).values # also  needs to be at least 1 ang from any pos O2 atom
            min_O2 = torch.cdist(cand_O2, atom_coords).min(dim=1).values # also  needs to be at least 1 ang from any pos O2 atom
            ...
            mins = torch.stack([min_center, min_O1, min_O2], dim=1)
            mins2 = torch.stack([min_center2, min_O1_2, min_O2_2], dim=1)
            ...
            for cen, perturb, in zip([gas_coords[0], center, gas_coords[1]], 
                                       [perturbed_gas_coords[0], perturbed, perturbed_gas_coords[1]]):
...
for j, (i, row) in enumerate(df.iloc[0:,].iterrows()):
    ...
    if row['in_row']:
        ...
        torch.save(pos_embedding, "tmp/_3Ppos_%d.pt"%count)
        ...
        torch.save(neg_embedding, "tmp/_3Pneg_%d.pt"%count)
